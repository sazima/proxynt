...
def _get_handshake_headers(
    ...
    headers = ["GET {} HTTP/1.1".format(resource), "Upgrade: websocket"]
    if port in [80, 443]:
        hostport = _pack_hostname(host)
    else:
        hostport = "{}:{}".format(_pack_hostname(host), port)
    if not options.get("suppress_host"):
        if options.get("host"):
            headers.append('Host: {}'.format(options["host"]))
        else:
            headers.append("Host: {}".format(hostport))
    ...
    if not options.get("suppress_origin"):
        if "origin" in options and options["origin"] is not None:
            headers.append('Origin: {}'.format(options["origin"]))
        elif scheme == "wss":
            headers.append("Origin: https://{}".format(hostport))
        else:
            headers.append("Origin: http://{}".format(hostport))

    key = _create_sec_websocket_key()

    # Append Sec-WebSocket-Key & Sec-WebSocket-Version if not manually specified
    if not options.get("header") or "Sec-WebSocket-Key" not in options["header"]:
        headers.append("Sec-WebSocket-Key: {}".format(key))
    else:
        key = options["header"]["Sec-WebSocket-Key"]

    if not options.get("header") or "Sec-WebSocket-Version" not in options["header"]:
        headers.append("Sec-WebSocket-Version: {}".format(VERSION))
    ...
    if subprotocols:
        headers.append('Sec-WebSocket-Protocol: {}'.format(",".join(subprotocols)))
    ...
    cookie_parts = []
    if server_cookie:
        cookie_parts.append(server_cookie)
    if client_cookie:
        cookie_parts.append(client_cookie)

    cookie = "; ".join(filter(None, cookie_parts))
    if cookie:
        headers.append("Cookie: {}".format(cookie))
...
def _get_resp_headers(
    ...
    if status not in success_statuses:
        ...
        raise WebSocketBadStatusException(
            "Handshake status {} {} -+-+- {} -+-+- {}".format(
                status, status_message, resp_headers, body_str
            ),
            status,
            status_message,
            resp_headers,
...
def _validate(headers: dict, key: str, subprotocols: Optional[List[str]]) -> Tuple[bool, Optional[str]]:
    subproto = None
    ...
    if subprotocols:
        ...
        if not subproto or subproto.lower() not in [s.lower() for s in subprotocols]:
            error("Invalid subprotocol: {}".format(subprotocols))
            return False, None
