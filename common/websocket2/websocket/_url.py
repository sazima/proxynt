...
def _is_no_proxy_host(hostname: str, no_proxy: Optional[list]) -> bool:
    if not no_proxy:
        env_no_proxy = os.environ.get("no_proxy", os.environ.get("NO_PROXY", "")).replace(" ", "")
        if env_no_proxy:
            no_proxy = env_no_proxy.split(",")
    ...
    if _is_ip_address(hostname):
        ...
        return any([_is_address_in_network(hostname, subnet) for subnet in subnets])
